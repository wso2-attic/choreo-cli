...
func startAuthCodeReceivingService(port int, oauth2Conf *oauth2.Config, setUserConfig config.SetConfig) (<-chan bool, *http.Server) {
	...
	mux.HandleFunc(callbackUrlContext, func(writer http.ResponseWriter, request *http.Request) {
		if err := request.ParseForm(); err != nil {
			message := "Login to Choreo failed due to an internal error. Please trying again."
		...
		if code == "" {
			message := "Login to Choreo failed due to an internal error. Please trying again."
			sendBrowserResponse(writer, http.StatusBadRequest, message)
			common.PrintErrorMessage("Login to Choreo failed due to receiving a blank auth code from IDP")
			oauthDone <- false
			return
		} else {
